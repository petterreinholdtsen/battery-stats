...
static void apm_fulldump(struct apm_info *ai)
{
    ...
    case AC_LINE_STATUS_OFF:
        printf(" off");
        break;
    case AC_LINE_STATUS_ON:
        printf(" on");
        break;
    case AC_LINE_STATUS_BACKUP:
        printf(" backup");
        break;
    default:
        printf(" unknown");
    ...
    if (ai->battery_flags & BATTERY_FLAGS_HIGH)
        printf("[HIGH]");
    if (ai->battery_flags & BATTERY_FLAGS_LOW)
        printf("[LOW]");
    if (ai->battery_flags & BATTERY_FLAGS_CRITICAL)
        printf("[CRITICAL]");
    if (ai->battery_flags & BATTERY_FLAGS_CHARGING)
        printf("[CHARGING]");
    if (ai->battery_flags & BATTERY_FLAGS_ABSENT)
        printf("[ABSENT]");
